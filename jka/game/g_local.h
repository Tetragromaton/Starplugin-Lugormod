...
#pragma once // for making sure of only one include
...
struct gentity_s {
	...
	int			timestamp;		// 1100 - body queue sinking, etc
	...
	int			count;
	int			bounceCount;
	qboolean	alt_fire; // 1416

	gentity_t	*chain; // 1420
	gentity_t	*enemy;
	gentity_t	*lastEnemy;
	gentity_t	*activator; // 1432
	...
	char* globalTarget;
	// StarPlugin skill cooldowns - Force user
	int fastRegenCd; // 1608
	int fastRegen; // 1612
	int ent_1616; // 1616
	int ent_1620; // 1620
	int ent_1624; // 1624
	// Skill cooldowns - Merc
	int treatInjuryCd; // 1628
	int ent_1632; // 1632
	int ent_1636; // 1636
	int ent_1640; // 1640
	int ent_1644; // 1644
...
typedef struct {
	team_t		sessionTeam; // 1768
	int			spectatorTime;		// for determining next-in-line to play
	spectatorState_t	spectatorState; // 1776
	int			spectatorClient;	// (1768) for chasecam and follow mode
	int			wins, losses;		// 1784, 1788 tournament stats
	...
	qboolean	teamLeader;			// 1808 true when this client is a team leader
	char		siegeClass[64]; // 1812
	char		saberType[64];
	char		saber2Type[64];
	int			duelTeam; // 2004
...
typedef struct {
	...
	qboolean	pmoveFixed;			//
	char		netname[MAX_NETNAME]; // 1600
	int			netnameTime;				// Last time the name was changed - 1744
	int			maxHealth;			// for handicapping - 1748
	int			enterTime;			// level.time the client entered the game - 1752
	playerTeamState_t teamState;	// status in teamplay games - 1756
	int			voteCount;			// to prevent people from constantly calling votes - 1800
	int			teamVoteCount;		// to prevent people from constantly calling votes - 1804
	qboolean	teamInfo;			// send team overlay updates? - 1808
	struct {
		int clientFlags;	// 1 = Shutup, 2 = Nocaps, 4 = Novote, 8 = King, 16 = ionlyduel, 32 = Hide Admin - 1812
		lmdDummy_t* lmd_1712;
		...
		int chatModeSay;	// none=0, all, team, admins, close, buddies, friends
		int chatModeTeam;
	} Lmd;
} clientPersistant_t;
...
typedef struct {
	...
	int			startTime;				// level.time the map was started - 40
...
void ItemUse_UseDiTrySpawn(gentity_t *ent, int type);
...
void trap_SetActiveSubBTrySpawn(int index);
